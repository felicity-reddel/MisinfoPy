...
def run_experiment(
    ...
    @param mlit_select: int in range [0, 100], %of agents selected for media literacy intervention
    @param rank_punish: int in range [-100, 0], visibility may be reduced by this %
    @param del_t: int in range [0, 100], p_true_treshold: if below -> may be deleted
    @param rank_t: int in range [0, 100], p_true_treshold: if below -> may be down-ranked
    @param strikes_t: int in range [0, 100], p_true_treshold: if below -> may be punished with a strike
    ...
    for _ in range(n_repl):
        ...
        model = MisinfoPy(
            ...
            rank_t=rank_t,
            strikes_t=strikes_t,
            belief_update_fn=belief_update,
            n_agents=n_agents,
...
def create_subplot(
    data, n_rows=3, n_cols=3, saving=False, title="", x_ticks=5, y_ticks=5, y_max=None
):
    """
    ...
    plot_ids = random.sample(
        all_run_ids, k=n_rows * n_cols
    )  # outside of model performance, can use non-model random

    # Create subplots
    fig, axes = plt.subplots(
        nrows=n_rows, ncols=n_cols, sharex="all", sharey="all", figsize=(10.8, 8)
    )
    ...
    if saving:
        ...
        fig.savefig(
            images_folder + f"belief_distributions_{title}.png",
            dpi=200,
            bbox_inches="tight",
            pad_inches=0.1,
        )
