...
def get_data(belief_update_fn, metric):
    ...
    """
    path = os.path.join(
        os.getcwd(), "data", "seedanalysis", belief_update_fn.name, "quantiles"
    )
...
def plot_quartiles(
    data, metric, title=None, y_min=0, y_max=600, fig_size=(10, 7), x_tick_freq=5
):
    """
    # TODO: FILL THIS
    @param data:
...
def plot_quartiles_metric_bufn_combo(
    metric, belief_update_fn, data_mapping, y_min=None, y_max=None
):
    ...
    plot_quartiles(
        data=data, metric=metric, title=belief_update_fn, y_min=y_min, y_max=y_max
    )
